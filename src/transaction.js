// ============================================================
// TRANSACTIONS PAGE SYNCHRONIZED JAVASCRIPT
// ============================================================
...
function setFilter(type) {
  ...
  ["all", "income", "expense"].forEach((f) => {
    ...
    const mBtn = document.getElementById(`m-filter-${f}`);

    if (btn) {
      if (f === type) {
        btn.className =
          "px-4 py-2 rounded-lg bg-white shadow-sm text-[#33B786]";
      } else {
        btn.className =
          "px-4 py-2 rounded-lg text-gray-500 hover:text-gray-800";
      }
    }

    if (mBtn) {
      if (f === type) {
        mBtn.className = "px-2 py-1 rounded bg-white text-[#33B786]";
      } else {
        mBtn.className = "px-2 py-1 rounded text-gray-500";
      }
    }
  });
